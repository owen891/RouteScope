...
真实 OneBot 验收使用 `scripts/onebot-uat.sh` 或 `scripts/onebot-uat.ps1`。默认是 dry-run；只有显式 `ONEBOT_CONFIRM=1` / `-Confirm` 才发送群聊、私聊和可选失败目标。输出只包含 HTTP、status、retcode 与 message_id 是否存在，不打印 access token。
